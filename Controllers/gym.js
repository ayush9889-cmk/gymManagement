...
exports.sendOtp = async (req, res) => {
  try {
    ...
    if (gym) {
      const buffer = crypto.randomBytes(4); // 4 bytes = 32-bit number
      const token = (buffer.readUInt32BE(0) % 900000) + 100000;
      gym.resetPasswordToken = token;
      gym.resetPasswordExpires = Date.now() + 3600000;
      await gym.save();
      const mailOption = {
        from: "ayushsingh723400@gmail.com",
        to: email,
        subject: "Password Reset",
        text: `You Requsted Password Reset . Your OTP is :${token}`,
      };
...
exports.logout = async (req, res) => {
  res
    .clearCookie("cookie_token", cookieOption)
