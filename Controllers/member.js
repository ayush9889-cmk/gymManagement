...
function addMonthsToDate(months, joiningDate) {
  ...
  const futurMonth = currentMonth + months;
  const futurYear = currentYear + Math.floor(futurMonth / 12);

  const adjustedMonth = futurMonth % 12;

  const futurDate = new Date(futurYear, adjustedMonth, 1);
...
exports.expiringWithin4To7Days = async (req, res) => {
  try {
    ...
    const member = await Member.find({
      gym: req.gym._id,
      nextBillDate: {
        $gte: next4Days, // Greater Than
        $lte: next7Days, // Less Than
      },
    });
